...
## Status: echte Sound-Dateien + Fortschritt-Cloud-Sicherung (Nachbesserungen zu Abschnitt 18e) – Stand 2026-09-05

**Echte Sounds statt Synthese:** Die selbst erzeugten Sinus-Töne klangen zu dünn. Ersetzt durch vier Dateien aus Kenneys "Interface Sounds"-Paket (kenney.nl, CC0/gemeinfrei, Lizenznachweis in `assets/sounds/KENNEY_LICENSE.txt`) – richtige Antwort, falsche Antwort, Button-Klick, Rundenende. Lokal in `assets/sounds/` gebündelt, nicht zur Laufzeit nachgeladen. Klick-Sound an `GameButton` verdrahtet (die wichtigsten Haupt-Buttons app-weit, z. B. "Spiel starten"/"Quiz-Duell starten" – nicht an jedem einzelnen Antwort-Button in den 17 Formaten, das hätte sich mit dem Richtig/Falsch-Sound direkt danach überschnitten), Rundenende-Sound an den gemeinsamen `ResultScreen`. Neue Stummschalt-Option in den Profil-Optionen, wichtig für gemeinsame Räume an Bord.

**Fortschritt zusätzlich in Firestore gesichert:** Bisher rein lokal (shared_preferences) – ging bei Geräte-/App-Wechsel oder Neuinstallation verloren. Jetzt zusätzlich Sicherung nach `users/{uid}.questionMastery`, nach demselben Offline-First-Prinzip wie der Rest der App: lokal bleibt die für den laufenden Betrieb genutzte Quelle (jede Antwort schreibt sofort lokal), im Hintergrund wird zusätzlich (wenn angemeldet) nach Firestore gesichert; die Fortschrittsanzeige gleicht zusätzlich bei jedem Öffnen lokalen und Cloud-Stand ab (pro Frage gewinnt die Seite mit mehr Versuchen) – so einmal gespielter Fortschritt auf einem neuen Gerät zurückgeholt wird, ohne neueren lokalen Fortschritt zu überschreiben.

**Dabei einen echten, sonst schwer zu findenden Bug vorgebeugt:** Fragen bekamen bisher keine feste ID – die Fortschrittsverfolgung nutzte `question.hashCode`. Das hätte den Cloud-Abgleich zwischen Plattformen zerstört: Darts `String.hashCode` liefert auf dart2js (Web) und der nativen VM/AOT (Android/Windows) unterschiedliche Werte für denselben String. Ein Nutzer, der auf dem Handy spielt und später die Web-Version öffnet, hätte auf jeder Plattform andere Schlüssel bekommen und nie zusammengeführten Fortschritt gesehen. Behoben, indem alle 37 Fragen in `assets/questions.json` jetzt eine stabile, mitgelieferte `id` (q01–q37) bekommen haben, die die Fortschrittsverfolgung statt des Hashes verwendet.

**Getestet:** `flutter analyze` clean, `flutter test` (38 Tests, davon 5 neue für den Cloud-Abgleich gegen `fake_cloud_firestore` – Sicherung, Wiederherstellung auf leerem lokalem Speicher, kein Überschreiben neueren lokalen Stands) und `flutter build web` laufen sauber; die vier Sound-Dateien wurden im Web-Build tatsächlich mitgebündelt (geprüft). Der reale Klang und das reale Sound-/Sync-Verhalten im Browser konnten von mir nicht geprüft werden (kein Audio-/Browser-Werkzeug in dieser Umgebung). **Noch nicht deployt** – die Firestore-Sicherheitsregeln erlauben `questionMastery` bereits (kein neues Feld in der Sperrliste nötig), aber wie immer erst nach explizitem Signal live schalten.

## Status: drei weitere Nachbesserungen (Grammatik, Ruder-/Leuchtturm-Icon) – Stand 2026-09-05

**1. Singular/Plural-Fehler ("1 Tage") behoben, systemweit geprüft.** Neue `S.plural(key, count, [args])`-Methode in `lib/l10n/strings.dart` (Singular-/Pluralform pro Sprache hinterlegt, wählt automatisch anhand von `count`) für die drei betroffenen Stellen in der übersetzten App-Hülle: Streak-Anzeige im Kopfbereich ("1 Tag" statt "1 Tage"), Tages-Challenge-Meldung, sowie neu auch die Lernfortschritt-Anzeige ("1 von 1 Frage sicher" statt fälschlich immer "Fragen" – hier hängt die Form vom zweiten Platzhalter ab, der Gesamtzahl, nicht vom Zähl-Argument selbst, das unterstützt die neue Methode ausdrücklich). Den ganzen `_values`-Block auf `{0}`-Platzhalter durchsucht: die übrigen Treffer (Runden-/Ergebnis-Anzeigen wie "Runde 2: 5 von 10 richtig", "Platz {0} diese Season") sind Ordinalzahlen oder reine Zahlenverhältnisse ohne mitgebeugtes Substantiv, brauchen also keine Pluralform. Zusätzlich zwei hartcodierte deutsche "Versuche"-Anzeigen in Match Up/Find the Match gefunden und mit einem kleinen lokalen Helfer (`lib/utils/german_plural.dart`) behoben – die beiden Format-Bildschirme bleiben wie beschlossen unübersetzt (Abschnitt 19), aber die deutsche Grammatik muss trotzdem stimmen.

**2. Ruder-Icon durch Steuerrad ersetzt.** Der zweite Korrekturversuch (deutlichere Ruderblätter) hat das Problem laut Rückmeldung nicht behoben – beide Blätter zeigten nach oben statt an gegenüberliegenden Enden, wirkte wie gekreuzte Löffel. Statt eines dritten Anlaufs direkt auf die selbst vorgeschlagene Alternative gewechselt: alle vier "1 vs 1"-Stellen (Reiter-Icon, Start-Hero, Leerzustand bei "Letzte Matches", Verlust-Anzeige im Match-Ergebnis) nutzen jetzt das Steuerrad-Motiv – dasselbe, das schon für die Wertungs-Anzeige steht (passt konzeptionell auch für "1 vs 1" selbst: Wettkampf/Können). Die jetzt ungenutzte `crossedOars`-Form inkl. Zeichenlogik komplett aus `maritime_icon.dart` entfernt statt als toter Code liegen zu bleiben.

**3. Leuchtturm-Icon kontrastreicher gezeichnet.** Bisheriger Entwurf: schmaler Turm (ein Drittel der Icon-Breite), zwei dünne 1.4px-Konturlinien als Streifen, dünne 1.2px-Strahlen-Linien – bei kleiner Darstellung (z. B. in der Streak-Statistik-Kachel) verschwanden die dünnen Details fast vollständig. Neu: breiterer Turm, ein einzelnes kräftiges gefülltes Band statt zweier dünner Linien, größere Laterne/Kuppel, und ein solider heller Lichtpunkt statt dünner Strahlen-Linien.

**Getestet:** `flutter analyze` clean, `flutter test` (42 Tests, davon 4 neue für `S.plural()`) und `flutter build web` laufen sauber. Die tatsächliche visuelle Wirkung der beiden Icon-Änderungen konnte ich weiterhin nicht selbst prüfen (kein Browser-Werkzeug in dieser Umgebung) – bitte im Browser gegenkontrollieren. **Noch nicht deployt** – reine Flutter/Client-Änderung.

## Status: "kein Ton" untersucht und behoben – Stand 2026-09-05

**Ursache gefunden, nicht im Flutter-Code, sondern im lokalen Test-Server:** `pubspec.yaml`-Registrierung war korrekt, die vier Sound-Dateien wurden auch tatsächlich vom Server ausgeliefert (per `curl` geprüft) – aber der lokale PowerShell-Testserver (`static_server.ps1`, dient nur zum lokalen Ansehen der Web-Builds, nicht Teil des Repos/der echten Auslieferung) hatte `.ogg` **nicht** in seiner MIME-Type-Tabelle und lieferte die Dateien deshalb als `application/octet-stream` statt `audio/ogg` aus – Chromes `<audio>`-Element verweigert die Wiedergabe bei einem generischen/falschen Content-Type. Zusätzlich fehlte jede Fehlerbehandlung pro Anfrage: eine `Range`-Anfrage (die `<audio>`-Elemente routinemäßig zur Wiedergabe-Vorbereitung schicken) ließ den ganzen Serverprozess mit einer unbehandelten Ausnahme abstürzen. Beides im Testserver behoben (MIME-Tabelle um `.ogg`/`.mp3`/`.wav`/weitere ergänzt, echte `Range`-Unterstützung mit 206-Antworten, Try/Catch pro Anfrage) und per `curl` verifiziert: korrekter `Content-Type: audio/ogg`, korrekte `206 Partial Content`-Antwort auf eine Range-Anfrage, Server übersteht das und bedient weitere Anfragen normal. **Das Firebase-Hosting-Deployment (Abschnitt 18d) wäre davon nicht betroffen gewesen** – ein echter Webserver setzt Content-Types korrekt; das Problem betraf ausschließlich die lokale Testumgebung.

**Zusätzlich robuster gemacht, unabhängig von der eigentlichen Ursache:** `lib/audio/sound_effects.dart` verschluckte Wiedergabefehler bisher komplett lautlos – jetzt werden sie per `debugPrint` geloggt (in der Browser-DevTools-Konsole sichtbar), damit sich "kein Ton" künftig leichter diagnostizieren lässt. Neue `SoundEffects.preload()` lädt alle vier Dateien beim App-Start bereits vor (nur die Quelle setzen, nichts abspielen – löst keine Autoplay-Sperre aus), damit der eigentliche `play()`-Aufruf beim ersten Antippen nicht mehr auf einen Netzwerk-Request warten muss, was in strengeren Browsern sonst dazu führen könnte, dass die Wiedergabe nicht mehr als direkte Folge der Nutzer-Interaktion erkannt und blockiert wird.

**Sprachausgabe (Hörverständnis-Format) separat abgesichert:** `flutter_tts`s Web-Implementierung wählt die Stimme synchron aus der Browser-Stimmenliste (`speechSynthesis.getVoices()`) - die lädt aber asynchron nach und ist beim allerersten Aufruf direkt nach dem Laden der Seite oft noch leer, wodurch "de-DE" fälschlich nicht gefunden würde. Behoben, indem die Sprache jetzt bei **jedem** Vorlese-Aufruf erneut gesetzt wird (nicht nur einmal beim Öffnen des Formats) - spätere Aufrufe treffen auf eine inzwischen geladene Stimmenliste. Zusätzlich: `setErrorHandler` zeigt jetzt eine sichtbare Meldung, wenn die Sprachausgabe tatsächlich fehlschlägt, ein Diagnose-Log beim Öffnen prüft (rein informativ, blockiert nichts), ob der Browser laut eigener Angabe eine deutsche Stimme hat, und die gesamte Sprachausgabe ist jetzt in try/catch gekapselt statt beim Fehlschlagen einfach nichts zu tun.

**Getestet:** `flutter analyze`, `flutter test` (42 Tests) und `flutter build web` laufen sauber; der Testserver-Fix wurde per `curl` gegen den tatsächlich laufenden lokalen Server verifiziert (Content-Type, Range-Antwort, Server-Stabilität). Das tatsächliche Hörergebnis im Browser (kommt jetzt wirklich Ton aus den Lautsprechern) konnte ich weiterhin nicht selbst prüfen (kein Audio-Ausgabe-Zugriff in dieser Umgebung) - bitte auf einem frischen Build/Hard-Reload gegenprüfen. **Noch nicht deployt** – reine Flutter/Client-Änderung, der Testserver-Fix betrifft ohnehin nur die lokale Entwicklungsumgebung.

## 18g. Bessere Sprachausgabe-Qualität (Hörverständnis-Format)

Die Sprachausgabe nutzt den TTS-Mechanismus des jeweiligen Geräts/Browsers (`flutter_tts`, kein eigener Cloud-Sprachdienst). Deren Qualität schwankt stark: netzbasierte Stimmen (z. B. Google, oder Microsofts "Online (Natural)"-Stimmen) klingen deutlich natürlicher als lokal auf dem Gerät installierte Standardstimmen (z. B. ältere Windows-SAPI-Stimmen) - bislang wählte die App aber einfach die erste zur eingestellten Sprache passende Stimme, ohne nach Qualität zu unterscheiden.

- **Kurzfristiger Teil (umgesetzt):** aus den verfügbaren deutschen Stimmen gezielt die beste auswählen, nicht die erste beste - Rangfolge: netzbasierte Google-Stimme > andere Neural-/Online-/Premium-Stimme > Standard-Systemstimme. Funktioniert nur mit dem, was das jeweilige Gerät/der Browser bereits an Stimmen mitbringt - keine Garantie, dass eine gute Stimme überhaupt verfügbar ist.
- **Langfristiger Teil (nicht jetzt):** ein eigener Cloud-Sprachdienst (z. B. Google Cloud Text-to-Speech, ElevenLabs) für garantiert gleichbleibend hohe Qualität unabhängig vom Gerät - würde aber laufende Kosten pro gesprochenem Zeichen verursachen und eine Internetverbindung zwingend voraussetzen (Konflikt mit dem Offline-First-Prinzip). Zurückgestellt, bis sich zeigt, ob die kurzfristige Lösung in der Praxis ausreicht.

## Status Abschnitt 18g, kurzfristiger Teil – Stand 2026-09-05

`flutter_tts`s `setLanguage()` wählt auf Web/den meisten Plattformen einfach die erste zur Sprache passende Stimme aus der Geräte-/Browser-Liste - ohne jede Qualitätsunterscheidung. Neue `rankTtsVoiceName()`-Funktion (`lib/utils/tts_voice_rank.dart`) ordnet Stimmen anhand ihres Namens ein (0 = enthält "google", 1 = enthält "neural"/"online"/"natural"/"premium"/"enhanced", 2 = alles andere) - eine einheitliche "ist das eine Netz-/Premium-Stimme"-Kennzeichnung liefert `flutter_tts` plattformübergreifend nicht, der Name ist das einzige verlässlich verfügbare Signal (Google/Microsoft benennen ihre besseren Stimmen durchgängig entsprechend).

`ListeningScreen._selectBestGermanVoice()` fragt beim Öffnen des Formats (und erneut bei jeder Frage, bis es einmal erfolgreich war - gleicher Grund wie beim vorherigen Fix: die Stimmenliste lädt asynchron nach) alle verfügbaren Stimmen ab, filtert auf Deutsch (`locale` beginnt mit "de"), sortiert nach der neuen Rangfolge und wählt die beste per `setVoice()` explizit aus - das überschreibt `setLanguage()`s naive Auswahl, die als sicherer Fallback weiterhin zuerst aufgerufen wird. Welche Stimme tatsächlich gewählt wurde (Name, Locale, Rang, Anzahl insgesamt verfügbarer deutscher Stimmen) wird per `debugPrint` geloggt, in der Browser-DevTools-Konsole sichtbar.

**Getestet:** neue `test/tts_voice_rank_test.dart` (4 Tests für die Rangfolge- und Sortierlogik) plus die volle bestehende Suite - 46 Tests insgesamt, `flutter analyze`/`flutter build web` clean. Die tatsächliche Stimmqualität und ob auf dem Testgerät überhaupt eine Google-/Neural-Stimme verfügbar ist, konnte ich nicht selbst hören (kein Audio-Zugriff in dieser Umgebung) - das Konsolen-Log sollte zeigen, welche Stimme gewählt wurde und wie viele deutsche Stimmen insgesamt zur Auswahl standen. **Noch nicht deployt** – reine Flutter/Client-Änderung.
