...
**Abschnitt 16/17 (Reiter-Navigation + Draft-Phase):** Das bisher gebaute **asynchrone** 1-vs-1-Matchmaking wurde nie live deployt und deshalb durch eine **echte Live-Warteschlange** ersetzt – die Draft-Phase braucht zwingend einen gerade anwesenden Gegner. Bans pro Spieler: 3 (6 insgesamt, abwechselnd), danach wählt jeder Spieler ein Format aus den verbleibenden 10, das dritte Best-of-3-Format wird zufällig aus den übrigen 8 gezogen. Reiter-Icons: einfache Flutter-Material-Icons als Platzhalter.

**Abschnitt 18 (Profil):** Nickname/Position/echter Name/Department/Avatar/Crew-ID/Deutsch-Level/Zertifikat. Sichtbarkeit serverseitig durchgesetzt (nicht nur UI-Versteck): `users/{uid}` bleibt per Regel nur für den jeweiligen Nutzer lesbar, eine Cloud Function spiegelt ausschließlich Nickname+Position in die öffentliche Rangliste. Das Deutsch-Level setzt einmalig die Start-Wertung (Level × 1000) über eine weitere Cloud Function – aber nur, solange noch kein gewertetes Match gespielt wurde (`hasPlayedRanked`, wie `eloRating` per Firestore-Regel vor Client-Schreibzugriff geschützt), damit niemand die eigene Wertung durch erneute Level-Angabe nach echten Matchergebnissen manipulieren kann. Zertifikat: nur Ausstellungsdatum eintragbar, Ablauf (+2 Jahre) und Gültigkeits-Status werden rein clientseitig berechnet, nicht gespeichert.

**Kompletter Ablauf im Firebase-Emulator mit echten Nutzer-Tokens getestet** (nicht per Admin-Umgehung, außer wo explizit ein echtes Matchergebnis simuliert wurde): Warteschlange → Zuordnung → Draft (Zug-Validierung, Bannen/Wählen, zufälliges drittes Format) → drei Runden mit Teil-Einreichungs-Handling → Sieger-Ermittlung → ELO-Update → Warteschlange-Reset → Ranglisten-Spiegel; Profilfelder speichern (Besitzer ja, `eloRating`/`hasPlayedRanked` client-seitig nein); Deutsch-Level setzt vor dem ersten Match korrekt die Start-Wertung, tut das nach einem simulierten Match nicht mehr; fremder Nutzer kann weder ein fremdes `users/{uid}`-Dokument lesen noch fremde Warteschlangen-/Match-Dokumente beschreiben; `careerRankings` enthält nie echten Namen, Department, Crew-ID, Level oder Zertifikatsdaten.
